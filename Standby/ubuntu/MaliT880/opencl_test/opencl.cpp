...
cl_device_id create_device() {
	...
	cl_uint dims;
	clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS, sizeof(cl_uint), &dims, 0);
	printf("Device dims: %d\n", dims);

	size_t max_items_size[3];
	clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_ITEM_SIZES, sizeof(size_t) * 3, max_items_size, 0);
	//printf("Device max_items_size: %d, %d, %d\n", (int)max_items_size[0], (int)max_items_size[1], (int)max_items_size[2]);
	printf("Device max_items_size: %d, %d, %d\n", max_items_size[0], max_items_size[1], max_items_size[2]);

	size_t max_group_size;
	clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(size_t), &max_group_size, 0);
	printf("Device max_group_size: %d\n", max_group_size);
...
cl_program build_program(cl_context context, cl_device_id device, const char* program_buffer1, const char* program_buffer2) 
{
	...
	const char *programs[] = {program_buffer1, program_buffer2};

	/* Create program from file */
	// xbdong: should send cnt & programs pointer
	//program = clCreateProgramWithSource(context, 1, (const char**)&program_buffer, NULL, &err);
	program = clCreateProgramWithSource(context, 2, (const char**)programs, NULL, &err);
	//if (err < 0) {
	if (err != CL_SUCCESS) {
