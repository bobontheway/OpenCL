...
本文介绍的模板模式是带有模板功能的模式，这里的模板和做月饼使用的模具功能类似。模板方法中的抽象方法在父类中声明，在子类中实现。不同的子类实现的具体处理不同，当父类的模板方法被调用时程序的行为也不同。但是，不管子类中的具体处理如何实现，处理流程都会按照父类中所定义的那样执行。这种在父类中定义处理流程，在子类中实现具体处理的模式称为（Template Method）模式。
