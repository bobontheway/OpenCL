...
int main()
{
	...
	// 将位图转换为 32 位
	// 问题：是否重新分配了内存？
	//FIBITMAT *image = FreeImage_ConvertTo32Bits(bitmap);
	//bitmap = FreeImage_ConvertTo32Bits(bitmap);
	bitmap_raw = FreeImage_ConvertTo32Bits(bitmap);

	get_image_property(bitmap_raw);
	printf("bitmap_raw=%p\n", bitmap_raw);
	get_image_property(bitmap);
	printf("bitmap=%p\n", bitmap);
	...
	get_image_property(bitmap);
	//bitmap = FreeImage_ConvertToType(bitmap, format) // FREE_IMAGE_TYPE
